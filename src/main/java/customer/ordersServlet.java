...
public class ordersServlet extends mainServlet {
	...
	private void createMain(PrintWriter out, HttpServletRequest request) {
		...
		out.println("<div class=\"row justify-content-center\">\r\n"
				...
				try {
					...
					ResultSet rs = stmt.executeQuery("SELECT * FROM `orders` WHERE customer_id = " + getUserID(request));
					while (rs.next()) {
						out.println("<div class=\"row justify-content-center\">\r\n"
	...
	private void showOrder(PrintWriter out, HttpServletRequest request) {
		...
		try {
			Statement stmt = DButil.getConnection(request).createStatement();
			ResultSet rs = stmt.executeQuery("SELECT products.book_name AS book_name, products.author_name AS author_name, products.picture_name AS picture_name,\r\n"
					+ "order_items.price AS price, order_items.quantity AS quantity\r\n"
					+ "FROM `order_items`\r\n"
					+ "INNER JOIN products ON (order_items.product_id = products.id)\r\n"
					+ "WHERE order_id = " + request.getParameter("id"));
			while (rs.next()) {
